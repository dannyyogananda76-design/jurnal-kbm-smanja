...
import type { Jadwal, Kelas, Mapel, TahunPelajaran } from '../../lib/types';
...
interface JadwalForm {
  kelas_id: string; mapel_id: string; tahun_pelajaran_id: string;
  hari: string; jam_ke_mulai: string; jam_ke_selesai: string;
...
export default function GuruJadwal() {
  ...
  const fetchData = async () => {
    if (!guru) return;
    setLoading(true);
    const [{ data: jadwal }, { data: kelas }, { data: mapel }, { data: tp }] = await Promise.all([
      supabase.from('jadwal').select('*, kelas(nama_kelas), mapel(nama_mapel), tahun_pelajaran(nama,semester)').eq('guru_id', guru.id).order('hari').order('jam_mulai'),
  ...
  useEffect(() => { fetchData(); }, [guru]);

  const filtered = filterHari ? jadwalList.filter(j => j.hari === filterHari) : jadwalList;

  const openCreate = () => {
    setEditId(null);
    const activeTp = tpList.find(t => t.aktif);
    setForm({
      kelas_id: '', mapel_id: '', tahun_pelajaran_id: activeTp?.id ?? '',
  ...
  const openEdit = (j: Jadwal) => {
    setEditId(j.id);
    setForm({
      kelas_id: j.kelas_id, mapel_id: j.mapel_id, tahun_pelajaran_id: j.tahun_pelajaran_id ?? '',
      hari: j.hari, jam_ke_mulai: String(j.jam_ke_mulai), jam_ke_selesai: String(j.jam_ke_selesai),
  ...
  const handleSave = async () => {
    if (!guru || !form.kelas_id || !form.mapel_id) {
      showToast('Kelas dan Mata Pelajaran wajib dipilih', 'error'); return;
    }
    setSaving(true);
    const payload = {
      guru_id: guru.id, kelas_id: form.kelas_id, mapel_id: form.mapel_id,
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    await supabase.from('jadwal').delete().eq('id', deleteId);
  ...
  const handleDeleteAll = async () => {
    if (!guru) return;
    if (!confirm('Hapus semua jadwal? Tindakan ini tidak bisa dibatalkan.')) return;
    await supabase.from('jadwal').delete().eq('guru_id', guru.id);
  ...
  const columns = [
    ...
    { key: 'waktu', label: 'Jam Mengajar', render: (r: Jadwal) => `${fmt(r.jam_mulai)} - ${fmt(r.jam_selesai)}` },
  ...
  return (
    <div>
      <h1 className="text-xl font-semibold text-gray-800 mb-5">Jadwal Mengajar</h1>

      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 flex-wrap gap-3">
          <h2 className="font-semibold text-gray-800">Data Jadwal Mengajar</h2>
          <div className="flex gap-2">
            <button onClick={handleDeleteAll} className="px-3 py-1.5 text-xs font-medium bg-red-100 hover:bg-red-200 text-red-700 rounded-lg transition-colors">
              Hapus Semua
            </button>
